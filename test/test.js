...
import { fasterPassword, fasterTenant, fasterUserName, partOrderNumber, workOrderNumber } from './config.js';
await describe('node-faster-report-exporter', async () => {
    ...
    await it('Exports a work order customer print', async () => {
        const reportPath = await reportExporter.exportWorkOrderCustomerPrint(workOrderNumber);
        console.log(reportPath);
        assert(reportPath !== undefined);
    });
    await it('Exports a work order technician print', async () => {
        const reportPath = await reportExporter.exportWorkOrderTechnicianPrint(workOrderNumber);
        console.log(reportPath);
        assert(reportPath !== undefined);
    });
});
